...
**domain:** coding_assistants and autonomous agent  
**model:** VON-3B (GGUF Q8_0)   
**created by:** Joseph Ayanda. This is my model.

this project is **two tracks in one model**: a coding assistant (write / repair code well) and an **agent** (short think, then tool calls, searches web if configured and all agentic coding). not only a chatbot
...
- **Base model:** `WeiboAI/VibeThinker-3B`. This is a continued 3B system, not a from-scratch pretrain. We started from that snapshot because the base model was fairly good at competitve programming(leetcode style stuff) , really good at maths and science but over-reasoned and couldnt call tools at all. We report the matched HumanEval lift because we were compute-constrained and this is the same-harness coding check we could finish, not because it is the official profiler score or a claim that this is the best possible result.
...
### training (+ evals) compute accross sft, rl, distillation:
...
| NVIDIA Tesla T4 (Kaggle)              | Parallel leftover coding-eval shards                           | **~150 GPU-hours** of weekly quota accross accts |
...
matched greedy HumanEval on the same 164-task EvalPlus 0.3.1 bank, same `WeiboAI/VibeThinker-3B` snapshot we started from, same generation settings (`max_new=8192`, greedy). it is extra same-harness coding evidence, not the ADTC profiler accuracy track. the model beats the base on both columns.
...
laptop `adtc-profiler` run of this same Q8_0 GGUF (`measured_on`: `participant_laptop`). Numbers below are taken from that `submission.json`.
